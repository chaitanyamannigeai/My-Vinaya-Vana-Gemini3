...
const AdminDashboard = () => {
  ...
  // --- LOCAL EDIT HELPERS ---
  const addRoomLocal = () => {
    const newRoom: Room = { id: `r${Date.now()}`, name: 'New Room', description: 'Desc...', basePrice: 3000, capacity: 2, amenities: ['Wifi'], images: ['https://images.unsplash.com/photo-1598928506311-c55ded91a20c?w=800'] };
    setRooms([newRoom, ...rooms]);
  };
  const saveRoom = async (id: string) => {
      const room = rooms.find(r => r.id === id);
      if (room) { try { await api.rooms.save(room); alert("Saved!"); } catch(e) { alert("Error"); } }
  };
  const deleteRoom = async (id: string) => {
      if (window.confirm("Delete?")) { try { await api.rooms.delete(id); setRooms(prev => prev.filter(r => r.id !== id)); } catch(e) { alert("Error"); } }
  };
  ...
  const saveDriver = async (id: string) => {
      ...
      if (d) { try { await api.drivers.save(d); alert("Saved!"); } catch(e) { alert("Error"); } }
  };
  const deleteDriver = async (id: string) => {
      if (window.confirm("Delete?")) { try { await api.drivers.delete(id); setDrivers(prev => prev.filter(d => d.id !== id)); } catch(e) { alert("Error"); } }
  };

  const addLocationLocal = () => {
      const newLoc: CabLocation = { id: Date.now().toString(), name: 'New Loc', description: '', imageUrl: 'https://images.unsplash.com/photo-1590664095612-2d4e5e0a8d7a?w=400', active: true, driverId: null, price: 0 };
      setLocations([newLoc, ...locations]);
  };
  const saveLocation = async (id: string) => {
      const l = locations.find(l => l.id === id);
      if (l) { try { await api.locations.save(l); alert("Saved!"); } catch(e) { alert("Error"); } }
  };
  const deleteLocation = async (id: string) => {
      if (window.confirm("Delete?")) { try { await api.locations.delete(id); setLocations(prev => prev.filter(l => l.id !== id)); } catch(e) { alert("Error"); } }
  };
  ...
  const savePricingRule = async (id: string) => {
      const r = pricingRules.find(r => r.id === id);
      if (r) { try { await api.pricing.save(r); alert("Saved!"); } catch(e) { alert("Error"); } }
  };
  const deletePricingRule = async (id: string) => {
      if (window.confirm("Delete?")) { try { await api.pricing.delete(id); setPricingRules(prev => prev.filter(r => r.id !== id)); } catch(e) { alert("Error"); } }
  };

  const addGalleryItemLocal = () => {
      const newItem: GalleryItem = { id: `g${Date.now()}`, url: 'https://images.unsplash.com/photo-1596176530529-78163a4f7af2?w=800', category: 'Property', caption: '' };
      setGallery([newItem, ...gallery]);
  };
  const saveGalleryItem = async (id: string) => {
      const g = gallery.find(g => g.id === id);
      if (g) { try { await api.gallery.save(g); alert("Saved!"); } catch(e) { alert("Error"); } }
  };
  const deleteGalleryItem = async (id: string) => {
      if (window.confirm("Delete?")) { try { await api.gallery.delete(id); setGallery(prev => prev.filter(g => g.id !== id)); } catch(e) { alert("Error"); } }
  };

  const addReviewLocal = () => {
      const newReview: Review = { id: `rev${Date.now()}`, guestName: 'Guest', location: '', rating: 5, comment: '', date: new Date().toISOString().split('T')[0], showOnHome: false };
      setReviews([newReview, ...reviews]);
  };
  const saveReview = async (id: string) => {
      const r = reviews.find(r => r.id === id);
      if (r) { try { await api.reviews.save(r); alert("Saved!"); } catch(e) { alert("Error"); } }
  };
  const deleteReview = async (id: string) => {
      if (window.confirm("Delete?")) { try { await api.reviews.delete(id); setReviews(prev => prev.filter(r => r.id !== id)); } catch(e) { alert("Error"); } }
  };

  const saveSettings = async () => {
      try { await api.settings.save(settings); alert("Saved!"); } catch(e) { alert("Error"); }
  };

  // --- RENDER FUNCTIONS (FULL) ---
  
  const renderBookings = () => (
    <div className="space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between">
                <div><p className="text-gray-500 text-sm font-medium">Total Revenue</p><h3 className="text-2xl font-bold text-gray-800 mt-1">₹{analytics.totalRevenue.toLocaleString()}</h3></div>
                ...
                {analytics.months.map(month => {
                    ...
                    return (
                        <div key={month} className="flex flex-col items-center gap-2 w-full group relative">
                            ...
                            <div className={`w-full max-w-[40px] rounded-t-md transition-all duration-500 relative ${value > 0 ? 'bg-nature-500 hover:bg-nature-600' : 'bg-gray-100'}`} style={{ height: `${heightPercent}%` }}></div>
            ...
            <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                    ...
                    {bookings.map(b => (
                        ...
                        <td className="px-6 py-4"><select value={b.status} onChange={(e) => updateBookingStatus(b.id, e.target.value as PaymentStatus)} className={`text-sm rounded-full px-3 py-1 font-semibold cursor-pointer border-none outline-none focus:ring-2 focus:ring-offset-1 ${b.status === 'PAID' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}><option value="PENDING">Pending</option><option value="PAID">Paid</option><option value="FAILED">Failed</option></select></td>
                        <td className="px-6 py-4"><a href={`https://wa.me/${b.guestPhone.replace(/[^0-9]/g, '')}?text=${encodeURIComponent(`Hi ${b.guestName}, greeting from Vinaya Vana Farmhouse!`)}`} target="_blank" rel="noreferrer" className="text-green-600 hover:text-green-800 flex items-center gap-1 text-sm font-medium"><MessageCircle size={18} /> Chat</a></td>
                        </tr>
                    ))}
                    </tbody>
                </table>
      ...
      {rooms.map(room => (
        <div key={room.id} className="bg-white p-6 rounded-lg shadow flex flex-col lg:flex-row gap-6 relative border border-gray-100">
            <div className="absolute top-4 right-4 flex gap-2 z-10">
                <button onClick={() => saveRoom(room.id)} className="flex items-center gap-1 bg-blue-600 text-white px-3 py-1 rounded shadow hover:bg-blue-700"><Save size={16} /> Save</button>
                <button onClick={() => deleteRoom(room.id)} className="text-red-400 hover:text-red-600 bg-white p-1 rounded border border-gray-200"><Trash2 size={20} /></button>
            </div>
            <div className="lg:w-1/3">
                 <ImageUploader label="Room Main Image" value={room.images[0]} onChange={(val) => setRooms(prev => prev.map(r => r.id === room.id ? { ...r, images: [val, ...r.images.slice(1)] } : r))} />
            </div>
            <div className="lg:w-2/3 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div><label className="block text-xs text-gray-500">Room Name</label><input type="text" value={room.name} onChange={(e) => setRooms(prev => prev.map(r => r.id === room.id ? { ...r, name: e.target.value } : r))} className="border rounded px-3 py-2 w-full font-bold"/></div>
                     <div><label className="block text-xs text-gray-500">Base Price (₹)</label><input type="number" value={room.basePrice} onChange={(e) => setRooms(prev => prev.map(r => r.id === room.id ? { ...r, basePrice: parseInt(e.target.value) } : r))} className="border rounded px-3 py-2 w-full"/></div>
                </div>
                <div><label className="block text-xs text-gray-500">Description</label><textarea value={room.description} onChange={(e) => setRooms(prev => prev.map(r => r.id === room.id ? { ...r, description: e.target.value } : r))} className="border rounded px-3 py-2 w-full h-20"/></div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                     <div><label className="block text-xs text-gray-500">Capacity</label><input type="number" value={room.capacity} onChange={(e) => setRooms(prev => prev.map(r => r.id === room.id ? { ...r, capacity: parseInt(e.target.value) } : r))} className="border rounded px-3 py-2 w-full"/></div>
                    <div><label className="block text-xs text-gray-500">Amenities (comma separated)</label><input type="text" value={room.amenities.join(', ')} onChange={(e) => setRooms(prev => prev.map(r => r.id === room.id ? { ...r, amenities: e.target.value.split(',').map(s => s.trim()) } : r))} className="border rounded px-3 py-2 w-full"/></div>
                </div>
            </div>
        </div>
      ))}
            ...
            {drivers.map(d => (
                <div key={d.id} className="bg-white p-4 rounded-lg shadow border-l-4 border-nature-500 relative">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
                        <div><label className="text-xs text-gray-500 block">Name</label><input type="text" value={d.name} onChange={(e) => setDrivers(prev => prev.map(drv => drv.id === d.id ? { ...drv, name: e.target.value } : drv))} className="border w-full p-1 rounded"/></div>
                        <div><label className="text-xs text-gray-500 block">Phone</label><input type="text" value={d.phone} onChange={(e) => setDrivers(prev => prev.map(drv => drv.id === d.id ? { ...drv, phone: e.target.value } : drv))} className="border w-full p-1 rounded"/></div>
                        <div><label className="text-xs text-gray-500 block">WhatsApp</label><input type="text" value={d.whatsapp} onChange={(e) => setDrivers(prev => prev.map(drv => drv.id === d.id ? { ...drv, whatsapp: e.target.value } : drv))} className="border w-full p-1 rounded"/></div>
                        <div className="flex items-center gap-4 pt-4">
                            <label className="flex items-center gap-2 text-sm cursor-pointer"><input type="checkbox" checked={d.isDefault} onChange={(e) => setDrivers(prev => prev.map(drv => drv.id === d.id ? { ...drv, isDefault: e.target.checked } : drv))} /> Default</label>
                             <label className="flex items-center gap-2 text-sm cursor-pointer"><input type="checkbox" checked={d.active} onChange={(e) => setDrivers(prev => prev.map(drv => drv.id === d.id ? { ...drv, active: e.target.checked } : drv))} /> Active</label>
                        </div>
                    </div>
                    <div className="mt-2"><label className="text-xs text-gray-500 block">Vehicle Info</label><input type="text" value={d.vehicleInfo || ''} onChange={(e) => setDrivers(prev => prev.map(drv => drv.id === d.id ? { ...drv, vehicleInfo: e.target.value } : drv))} className="border w-full p-1 rounded"/></div>
                </div>
            ))}
        </div>
    </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {locations.map(loc => (
                <div key={loc.id} className="bg-white p-4 rounded-lg shadow relative border border-gray-100">
                    <div className="absolute top-2 right-2 flex gap-2 z-10"><button onClick={() => saveLocation(loc.id)} className="bg-white text-blue-600 p-1 rounded shadow hover:bg-blue-50"><Save size={16}/></button><button onClick={() => deleteLocation(loc.id)} className="bg-white text-red-500 hover:bg-red-50 p-1 rounded shadow"><Trash2 size={16}/></button></div>
                    <div className="mb-4"><ImageUploader label="Location Image" value={loc.imageUrl} onChange={(val) => setLocations(prev => prev.map(l => l.id === loc.id ? { ...l, imageUrl: val } : l))} /></div>
                    <div className="space-y-2">
                         <input type="text" value={loc.name} onChange={(e) => setLocations(prev => prev.map(l => l.id === loc.id ? { ...l, name: e.target.value } : l))} className="font-bold border w-full p-1 rounded" placeholder="Location Name"/>
                         <textarea value={loc.description} onChange={(e) => setLocations(prev => prev.map(l => l.id === loc.id ? { ...l, description: e.target.value } : l))} className="text-sm border w-full p-1 rounded h-20" placeholder="Description"/>
                        <div className="flex gap-2">
                            <div className="w-1/2"><label className="text-xs block text-gray-500">Price</label><input type="number" value={loc.price ?? ''} onChange={(e) => { const val = e.target.value === '' ? 0 : parseFloat(e.target.value); setLocations(prev => prev.map(l => l.id === loc.id ? { ...l, price: val } : l)); }} className="border w-full p-1 rounded"/></div>
                            <div className="w-1/2"><label className="text-xs block text-gray-500">Driver</label><select value={loc.driverId || ''} onChange={(e) => setLocations(prev => prev.map(l => l.id === loc.id ? { ...l, driverId: e.target.value || null } : l))} className="border w-full p-1 rounded text-sm"><option value="">Default Driver</option>{drivers.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}</select></div>
                        </div>
                    </div>
                </div>
            ))}
            ...
            {pricingRules.map(rule => (
                <div key={rule.id} className="bg-white p-4 rounded-lg shadow flex flex-col md:flex-row items-center gap-4 relative">
                    ...
                    <div className="flex-grow grid grid-cols-1 md:grid-cols-4 gap-4 w-full mt-2 md:mt-0">
                        <div><label className="text-xs text-gray-500">Season Name</label><input type="text" value={rule.name} onChange={(e) => setPricingRules(prev => prev.map(r => r.id === rule.id ? { ...r, name: e.target.value } : r))} className="border w-full p-2 rounded"/></div>
                        <div><label className="text-xs text-gray-500">Start Date</label><input type="date" value={rule.startDate} onChange={(e) => setPricingRules(prev => prev.map(r => r.id === rule.id ? { ...r, startDate: e.target.value } : r))} className="border w-full p-2 rounded"/></div>
                        <div><label className="text-xs text-gray-500">End Date</label><input type="date" value={rule.endDate} onChange={(e) => setPricingRules(prev => prev.map(r => r.id === rule.id ? { ...r, endDate: e.target.value } : r))} className="border w-full p-2 rounded"/></div>
                        <div><label className="text-xs text-gray-500">Multiplier</label><input type="number" step="0.1" value={rule.multiplier} onChange={(e) => setPricingRules(prev => prev.map(r => r.id === rule.id ? { ...r, multiplier: parseFloat(e.target.value) } : r))} className="border w-full p-2 rounded font-bold text-nature-700"/></div>
                    </div>
                </div>
            ))}
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {gallery.map(item => (
                  <div key={item.id} className="bg-white p-4 rounded-lg shadow relative group border border-gray-100">
                      ...
                      <div className="mb-3"><ImageUploader value={item.url} onChange={(val) => setGallery(prev => prev.map(g => g.id === item.id ? { ...g, url: val } : g))} /></div>
                      <div className="space-y-2">
                          <div><label className="text-xs text-gray-500">Category</label><input type="text" value={item.category} onChange={(e) => setGallery(prev => prev.map(g => g.id === item.id ? { ...g, category: e.target.value } : g))} className="border w-full p-1 rounded text-sm"/></div>
                          <div><label className="text-xs text-gray-500">Caption</label><input type="text" value={item.caption || ''} onChange={(e) => setGallery(prev => prev.map(g => g.id === item.id ? { ...g, caption: e.target.value } : g))} className="border w-full p-1 rounded text-sm"/></div>
                      </div>
                  </div>
              ))}
          </div>
            ...
            {reviews.map(rev => (
                <div key={rev.id} className="bg-white p-6 rounded-lg shadow border-l-4 border-yellow-400 relative">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        <div><label className="text-xs text-gray-500">Guest Name</label><input type="text" value={rev.guestName} onChange={(e) => setReviews(prev => prev.map(r => r.id === rev.id ? { ...r, guestName: e.target.value } : r))} className="border w-full p-1 rounded font-bold"/></div>
                        <div><label className="text-xs text-gray-500">Location</label><input type="text" value={rev.location} onChange={(e) => setReviews(prev => prev.map(r => r.id === rev.id ? { ...r, location: e.target.value } : r))} className="border w-full p-1 rounded"/></div>
                        <div><label className="text-xs text-gray-500">Rating (1-5)</label><input type="number" min="1" max="5" value={rev.rating} onChange={(e) => setReviews(prev => prev.map(r => r.id === rev.id ? { ...r, rating: parseInt(e.target.value) } : r))} className="border w-full p-1 rounded"/></div>
                    </div>
                    <div className="mb-4"><label className="text-xs text-gray-500">Comment</label><textarea value={rev.comment} onChange={(e) => setReviews(prev => prev.map(r => r.id === rev.id ? { ...r, comment: e.target.value } : r))} className="border w-full p-2 rounded h-20 text-sm"></textarea></div>
                    <div className="flex items-center justify-between">
                        <div><label className="text-xs text-gray-500">Date</label><input type="date" value={rev.date} onChange={(e) => setReviews(prev => prev.map(r => r.id === rev.id ? { ...r, date: e.target.value } : r))} className="border ml-2 p-1 rounded"/></div>
                        <div className="flex items-center gap-2"><input type="checkbox" checked={rev.showOnHome} onChange={(e) => setReviews(prev => prev.map(r => r.id === rev.id ? { ...r, showOnHome: e.target.checked } : r))} id={`home-${rev.id}`} /><label htmlFor={`home-${rev.id}`} className="text-sm font-medium cursor-pointer">Show on Home Page</label></div>
                    </div>
                </div>
            ))}
        </div>
  ...
  const renderHomePageContent = () => (
    <div className="bg-white p-8 rounded-lg shadow max-w-3xl space-y-8">
        ...
        <div className="bg-nature-50 p-4 rounded-lg border border-nature-200">
            <h3 className="text-lg font-bold mb-2 flex items-center gap-2 text-nature-900"><BarChart2 size={20}/> Website Traffic</h3>
        ...
        <button onClick={saveSettings} className="flex items-center gap-2 bg-nature-600 text-white px-6 py-2 rounded-md hover:bg-nature-700 w-full justify-center"><Save size={18} /> Save Content</button>
    </div>
  );
            ...
            <div className="space-y-6">
                <div>
                     ...
                     <div className="bg-white p-4 rounded border border-gray-200 space-y-4">
                        <div className="flex items-center gap-2"><input type="checkbox" id="enableDiscount" checked={settings.longStayDiscount?.enabled ?? true} onChange={(e) => setSettings({ ...settings, longStayDiscount: { ...settings.longStayDiscount, enabled: e.target.checked } })} /><label htmlFor="enableDiscount" className="text-sm font-medium">Enable Long Stay Discount</label></div>
