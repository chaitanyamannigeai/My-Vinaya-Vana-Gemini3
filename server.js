...
// --- 1. ROBUST PATH RESOLUTION (CRITICAL FOR DOCKER) ---
// We use process.cwd() to find the folder where the server actually runs
...
const fixDatabaseSchema = async () => {
    try {
        ...
        // 4. Migrations (Updates to existing tables)
...
// --- 6. API ROUTES (VEHICLES, REVIEWS, ETC) ---
...
// Reviews, Pricing, Rooms, Drivers, Locations, Gallery
...
app.post('/api/analytics/track-hit', async (req, res) => { 
    try { 
        ...
        if (ip && ip.length > 7) { 
            ...
            } catch (geoError) { console.error("GeoIP Error:", geoError.message); }
        }
...
app.get('/api/health', async (req, res) => { try { const connection = await pool.getConnection(); connection.release(); res.json({ status: 'OK', database: 'Connected' }); } catch (err) { res.status(500).json({ status: 'ERROR', error: err.message }); } });

// --- 7. THE UI FIX: ROBUST STATIC SERVING (CRITICAL) ---

// Verify Dist Folder Exists
if (fs.existsSync(DIST_PATH)) {
    // A. Explicitly serve /assets from dist/assets
    // This stops the "MIME Type Mismatch" dead in its tracks.
    app.use('/assets', express.static(path.join(DIST_PATH, 'assets'), {
        maxAge: '1y',
        immutable: true, 
    }));

    // B. Serve the rest of the static files (favicon, etc.)
    app.use(express.static(DIST_PATH, {
        maxAge: '1y',
        etag: true,
    }));

    // C. SPA Fallback (Index.html for unknown routes)
    app.get('*', (req, res) => {
        // Optimization: Don't serve HTML for missing images/css, send 404 instead
    ...
    // Error Page if Build Failed
    app.get('*', (req, res) => res.send('<h1>Backend Running</h1><p>Frontend dist folder not found. Check Northflank Build Logs.</p>'));
}
