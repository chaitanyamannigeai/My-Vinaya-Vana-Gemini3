...
// --- DATABASE STARTUP FIXER (ENHANCED FOR PHASE 2) ---
const fixDatabaseSchema = async () => {
    try {
        ...
        // 2. FIX BOOKINGS TABLE (ADD BALANCE COLUMNS)
        try {
            // Check if columns exist, if not ADD them
            const [cols] = await connection.query("SHOW COLUMNS FROM bookings LIKE 'amount_paid'");
            if (cols.length === 0) {
                console.log("🔧 Adding partial payment columns to bookings...");
                await connection.query("ALTER TABLE bookings ADD COLUMN amount_paid DECIMAL(10,2) DEFAULT 0");
                await connection.query("ALTER TABLE bookings ADD COLUMN balance_amount DECIMAL(10,2) DEFAULT 0");
                // Update existing bookings to have consistent data
                await connection.query("UPDATE bookings SET amount_paid = total_amount, balance_amount = 0 WHERE status = 'PAID'");
            }
        } catch (e) {
            console.error("Failed to update bookings schema:", e.message);
        }
...
app.post('/api/pricing', async (req, res) => {
    ...
    try {
        ...
        } catch (fatalError) { res.status(500).json({error: fatalError.message}); }
    }
});
...
// --- BOOKINGS (UPDATED FOR PARTIAL PAYMENTS) ---

app.get('/api/bookings', async (req, res) => { 
    try { 
        // Select specific columns to handle cases where new columns might briefly not exist
        ...
        res.json(rows.map(b => ({ 
            ...
            amountPaid: b.amount_paid || b.total_amount, // Fallback for old records
            balanceAmount: b.balance_amount || 0,        // Fallback
            status: b.status, 
...
// Single Booking Fetch (For Pay Balance Page)
...
app.post('/api/bookings', async (req, res) => { 
    try { 
        ...
        const query = `
            ...
            (id, room_id, guest_name, guest_phone, check_in, check_out, total_amount, amount_paid, balance_amount, status) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) 
            ON DUPLICATE KEY UPDATE 
            status=VALUES(status), amount_paid=VALUES(amount_paid), balance_amount=VALUES(balance_amount)
        `;

        await pool.query(query, [id, roomId, guestName, guestPhone, checkIn, checkOut, total, paid, balance, finalStatus]); 
...
// Update Status / Manual Payment
app.put('/api/bookings/:id', async (req, res) => { 
    try { 
        // If updating status to PAID, assume full payment collected
        if (req.body.status === 'PAID') {
             await pool.query(`
                UPDATE bookings 
                SET status = 'PAID', amount_paid = total_amount, balance_amount = 0 
                WHERE id = ?`, 
            [req.params.id]);
        } else {
...
app.post('/api/bookings/:id/pay-balance', async (req, res) => {
    try {
        ...
        const total = parseFloat(rows[0].total_amount || 0);
        
        const newPaid = currentPaid + paymentAmount;
        const newBalance = total - newPaid;
        const newStatus = newBalance <= 1 ? 'PAID' : 'PARTIAL'; // Tolerance for float math

        await pool.query(
            'UPDATE bookings SET amount_paid = ?, balance_amount = ?, status = ? WHERE id = ?',
