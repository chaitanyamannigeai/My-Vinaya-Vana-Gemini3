...
// --- 1. ROBUST PATH RESOLUTION (Fixes Docker/Cloud Path Issues) ---
// Using process.cwd() is safer than __dirname in cloud containers
...
// Critical Debugging: Check if dist exists to prevent "White Screen"
if (fs.existsSync(DIST_PATH)) {
    console.log('✅ Dist folder found!');
    try {
        if (fs.existsSync(path.join(DIST_PATH, 'assets'))) {
            console.log('✅ Assets folder found inside dist');
        } else {
            console.error('⚠️ CRITICAL: Assets folder MISSING in dist. Build might be incomplete.');
        }
    } catch (e) {
...
// Trust proxy for Render/Northflank load balancers
...
app.use(helmet({
    contentSecurityPolicy: {
        directives: {
            ...
            // Allows images from your domain and external sources (Pichwai art)
...
// --- 8. UI FALLBACK (ROBUST STATIC SERVING) ---

if (fs.existsSync(DIST_PATH)) {
    // A. Explicitly serve /assets from dist/assets to prevent fallback errors
    ...
    // B. Serve the rest of the static files (favicon, manifest, etc.)
    ...
    // C. SPA Fallback: Redirect unknown routes to index.html
    app.get('*', (req, res) => {
        // Optimization: If the browser asks for a missing CSS/JS file, send 404 instead of HTML
    ...
    app.get('*', (req, res) => res.send('<h1>Backend Running</h1><p>Frontend dist folder not found. Check Server Logs.</p>'));
}
